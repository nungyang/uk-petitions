...
async def main():
    ...
    cut_off = datetime.now().date() - relativedelta(months=12)
    recent_closed_petitions = all_closed_petitions[all_closed_petitions['closed_at'] >= cut_off]
    print(f"   Found {len(recent_closed_petitions)} petitions closed in last 12 months")
    
    print("   Fetching constituency data for petitions closed in last 12 months...")
    ...
    upload_to_s3(all_petitions_list, f'dynamic data/dashboard_list.csv', s3_client, bucket)
    upload_to_s3(all_petitions_counts, f'dynamic data/dashboard_counts.csv', s3_client, bucket)
